...
export const performanceHandlers = [
  http.get("/api/performance", ({ request }) => {
    ...
    const data = performanceData.slice(-days);
    return HttpResponse.json({ data });
  }),

  http.get("/api/performance/kpi", () => {
    ...
    return HttpResponse.json({
      data: [
        { title: "Conversion Rate", value: avgConversion, change: ((avgConversion - prevConversion) / prevConversion) * 100, changeLabel: "vs last 30 days" },
        { title: "New Customers", value: newCustomers, change: ((newCustomers - prevNew) / prevNew) * 100, changeLabel: "vs last 30 days" },
        { title: "Returning Customers", value: returning, change: ((returning - prevReturning) / prevReturning) * 100, changeLabel: "vs last 30 days" },
      ],
    });
  }),
];
